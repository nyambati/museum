...
export const deleteChart = chart => dispatch =>
  axios
    .delete(`api/charts/${chart.name}?version=${chart.version}`)
    .then(({ data }) => {
      if (data.message) return dispatch(deleteChartError(data.message));
      dispatch(deleteChartSuccess(chart));
    })
    .catch(error => dispatch(deleteChartError(error.message)));
